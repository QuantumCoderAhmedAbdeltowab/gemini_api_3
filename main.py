...
if not GOOGLE_API_KEY:
    raise ValueError(
        "GOOGLE_API_KEY environment variable is not set. Please set it to your API key."
    )

genai.configure(api_key=GOOGLE_API_KEY)
gemini_model = genai.GenerativeModel("gemini-2.0-flash")

data = pd.read_csv("phishaware_sheet.csv")
...
def ask_question(q: Question):
    ...
    if similarity_score > 0.8:
        ...
        try:
            ...
            return {"error": "Failed to generate response from Gemini model.", "details": str(e)}
